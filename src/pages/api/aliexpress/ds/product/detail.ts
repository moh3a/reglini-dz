              ...
              async (error: IAEError, response: IBasicProductDetails) => {
                if (!error) {
                  ...
                  if (
                    ...
                    response.result.ae_item_sku_info_dtos.ae_item_sku_info_d_t_o
                  ) {
                    response.result.ae_item_sku_info_dtos?.ae_item_sku_info_d_t_o.map(
                      (variations) => {
                        if (
                          variations.ae_sku_property_dtos &&
                          variations.ae_sku_property_dtos.ae_sku_property_d_t_o
                        ) {
                            ...
                            (props) => {
                              ...
                              if (index !== -1) {
                                const exits = properties[index].values.find(
                                  (e) =>
                                    e.id === props.property_value_id?.toString()
                                );
                                if (!exits)
